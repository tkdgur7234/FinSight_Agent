...
3. 이상 거래 감지
   ...
   - 작년 데이터는 정 대형 종목에 한해 DB(SQLite)를 통한 1년치 가 데이터 생성
...
Z-score: yfinance로 해당 종목의 작년~현재 일봉 데이터를 가져와 통계적 이상치(Z-score)를 계산
작년의 데이터는 특정 대형 종목에 한해 DB(SQLite)를 통한 1년치 가 데이터 생성
주간/월간(고래 출몰) 빈도수 + Z-score 리포팅

seen_ticker 정확히 어떻게 작동?

(25-2-1) 3. 이상 거래 감지 기능
(1) 3-1. 대규모 거래 감지 - 크롤링 로직 수정
수정된 코드 잘봤어. 
1. 웹사이트 html 구성시 1차 필터링된 종목을 먼저 보여주고 2차 필터링 종목을 그 밑에 보여줘. 2차 필터링까지 되었으면 1차 필터링은 안보여줘도 돼. 이렇게 수정 부탁해. 
2. 

(2) 3-2. 주요 종목 내부자 거래 감시 - 로직
(3) 2, 3 과정 html 병합


3. 기존의 3-2. 주요 종목 내부자 거래 감시 기능을  새파일로 만들거야. 기존 기능은 다음과 같아. 
Nasdaq 100, S&P 500 등 주요 기업 임원들의 미공개 정보 기반 거래 패턴 분석
 지난 24시간동안 새로 올라온 모든 공시 긁어오기
 데이터 필터링 : 장내 매수와 단순 매도만 인정 (스톡옵션 행사 후 당일 매도는 취급X)
 매수 집중도 : 직급에 따른 가중치(RoleWeight)를 부여하고 한달 내 3명 이상 매수시 Cluster Buy
