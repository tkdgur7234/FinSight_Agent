...
(25-2-6) 3. 이상 거래 감지 기능  
(1) 3-1. 대규모 거래 감지 - 로직 수정  
대규모 거래 데이터 수집시 개별 종목과 etf 나눠서 수집하기.  
포스팅도 마찬가지  
google처럼 snp와 nasdaq에 모두 포함되어있는 종목의 경우 중복 배제 로직  
데이터 수집시 : rel volume > 1.5인 종목들을 거래량(volume)순으로 db 기록

(25-2-7)  
(1) 3-2. 주요 종목 내부자 거래 감시 - 로직 수정  
Finviz는 스톡 옵션 행사 후 매도를 파악할 컬럼이 없기에 Openinsider의 S - Sale+OE 컬럼 사용.  
한달 전까지의 내부자 거래 데이터 수집 (최대 2000건)  
종목명, 내부자 매수 건수 리포팅 + 잡주 필터링 로직(주가 3천원, 시총 3천억, 거래대금 30억 이상)  
1차 필터링 : 장내 매수, 단순 매도만 허용하고 동전주 방지로 주가 $3, 매수 $150k, 매도 $500k 이상만 필터링  
2차 필터링 : 재무 건전성 필터링 (시총 3500억, 거래대금 35억 이상)  
리포팅 : 집단 매수(최근 30일 내 3명 이상 매수), 주요 매수(150k 이상 매수, c-level일 때), 대량 매도(500k 이상) 나눠서 포스팅

(25-2-8)  
(1) 3-2. 주요 종목 내부자 거래 감시 - 로직 수정  
크롤링 기준이 너무 타이트함. 좀 더 매수, 매도 금액 기준을 느슨하게 변경  
크롤링 기준을 타이트하게 가져오려고 했는데 사이트 내에서 계속 엉키는 것 같음. (최근 한달치 데이터 -> 필터링)  
-> 먼저 5천개만 가져오고 파이썬 코드 내에서 엄격 필터링 (5천개에서 한달치 데이터만 필터링)  
성능 최적화 진행 : 기존엔 필터된 데이터를 순차적으로 yfinance 서버와 통신해서 300개만 해도 3분이었는데,  
멀티스레딩(ThreadPoolExecutor 이용)해 15초로 단축.  
(2) 2, 3 과정 html 병합  
3-1 Finviz(링크포함)의 Rel Volume으로 1차 포착 후, 통계적 이상치(Z-score) 분석을 통해 단순 변동성과 세력의 개입을 구분합니다.  
3-2 티커, 분류 유형, 집단매수거나 주요 매수라면 총 거래금과 직급 명시

---

👑 헤드라인 (가장 중요): Cluster Buy (집단 매수)

조건: 최근 1개월 내 3명 이상의 임원이 매수한 종목.

표시: 최상단에 🔥 아이콘과 함께 강조 박스로 표시.

의미: "임원진들이 단체로 우리 회사 싸다고 줍고 있다." (가장 강력한 호재)

💎 주요 매수 (Significant Buys)

조건: Cluster는 아니지만, 거래 금액이 $100,000(약 1.3억) 이상이거나 **C-Level(CEO, CFO)**의 매수.

표시: 그 아래에 리스트 형태로 표시.

의미: "사장님이 제돈 주고 샀다."

📉 주요 매도 (Big Sells) - 필터링 필수

조건: 매도는 너무 흔하므로 기준을 높입니다. $500,000(약 6.5억) 이상의 대량 매도만 표시.

표시: 하단에 작게 표시하거나, 별도 탭으로 분리.

필터: $50,000 이하의 소액 매도(생활비 마련 등)는 아예 수집 단계에서 버립니다.
